...
class Clope
{
    ...
    function clustering ()
    {
        ...
        foreach ($transactions as $transaction) {
            $bestClusterId = $this->getBestClusterId($transaction, 
                    $this->repulsion);
            if (is_null($bestClusterId)) {
                $this->cluster_id = $this->ClopeDB->clusterNewAddTransaction(
                        $transaction['id']);
            } else {
        ...
        do {
            foreach ($transactions as $transaction) {
                ...
                if (is_null($bestClusterId)) {
                    ...
                    $this->changes_issue = $this->transactionMoveToCluster(
                            $transaction, $bestClusterId);
                }
            }
    ...
    public function transactionMoveToCluster ($transaction, $bestClusterId)
    {
        ...
        $transactionIsFree = $this->ClopeDB->transactionIsFree(
                $transaction['id']);
        if ($transactionIsFree) {
            $clusterIdFrom = null;
            $this->ClopeDB->clusterTransactionMoveToCluster($clusterIdFrom, 
                    $clusterIdTo, $transactionId);
            return true;
        } else {
            $clusterIdFrom = $this->ClopeDB->transactionGetClusterId(
                    $transaction['id']);
    ...
    public function getBestClusterId ($transaction, $repulsion)
    {
        ...
        if (! is_null($clusters)) {
            foreach ($clusters as $cluster) {
                if ($this->ClopeDB->clusterCheckTransactionExist($cluster['id'], 
                        $transaction['id'])) {
                    $delta = $this->deltaRemove($cluster, $transaction, 
                            $repulsion);
                } else {
    ...
    {
        $transactionItems = $this->ClopeDB->getTransactionItems(
                $transaction['id']);
        $clusterItems = $this->ClopeDB->getClusterItems($cluster['id']);
        
        $clusterCountTransactions = $this->ClopeDB->getCountTransactionsOfCluster(
                $cluster['id']);
        $transactionExist = $this->ClopeDB->clusterCheckTransactionExist(
                $cluster['id'], $transaction['id']);
        ...
        $paramNew = $sizeNew * ($clusterCountTransactions + 1) /
                 pow($widthNew, $this->repulsion);
        $param = $cluster['size'] * $clusterCountTransactions /
                 pow($cluster['width'], $this->repulsion);
